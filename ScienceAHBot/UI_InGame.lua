...
function M.update_items_tab(root, bx, by, bw, bottomY, click, cx, cy)
  ...
  local function z(n)
    return (n or 0) * (root._uiScale or 1)
  end
  local rowH = z(ITEM_ROW_H)
  local listTop = by + z(LIST_TOP_OFFSET)
  local listH = math.max(z(80), bottomY - listTop - z(8))
  local ids = M.sorted_item_ids(cfg)

  if click and inside(cx, cy, bx, by + z(36), bw, z(22)) then
    root.uiFocus = "addId"
    root._uiKeyPrev = {}
  end
  ...
  if inside(cx, cy, bx, listTop, bw, listH) then
    ...
    if wd ~= 0 then
      local total = #ids * rowH
      local maxScroll = math.max(0, total - listH)
      root.itemsScroll = math.max(0, math.min(maxScroll, (root.itemsScroll or 0) - wd * z(20)))
    end
  end
  ...
  if inside(cx, cy, bx, by + z(62), z(132), z(28)) then
    M.merge_starter_items(cfg)
  end
  if inside(cx, cy, bx + z(140), by + z(62), z(88), z(28)) then
    M.clear_all_items(cfg)
  end

  if inside(cx, cy, bx + bw - z(160), by + z(62), z(36), z(28)) then
    root._uiNewItemRatio = clamp((root._uiNewItemRatio or 0.75) - 0.05, 0.05, 0.99)
  end
  if inside(cx, cy, bx + bw - z(118), by + z(62), z(36), z(28)) then
    root._uiNewItemRatio = clamp((root._uiNewItemRatio or 0.75) + 0.05, 0.05, 0.99)
  end

  if inside(cx, cy, bx + bw - z(200), by + z(94), z(200), z(30)) then
  ...
  for i = 1, #ids do
    local id = ids[i]
    local ry = listTop + (i - 1) * rowH - scroll
    if ry + rowH >= listTop and ry <= listTop + listH then
      if inside(cx, cy, bx + bw - z(40), ry + z(10), z(32), z(26)) then
        cfg.Items[id] = nil
      end
      if inside(cx, cy, bx + z(188), ry + z(10), z(36), z(26)) then
        local row = cfg.Items[id]
        if row then
          row.ratio = clamp((row.ratio or 0.75) - 0.05, 0.05, 0.99)
        end
      end
      if inside(cx, cy, bx + z(230), ry + z(10), z(36), z(26)) then
        local row = cfg.Items[id]
        if row then
          row.ratio = clamp((row.ratio or 0.75) + 0.05, 0.05, 0.99)
        end
...
function M.update_setup_tab(root, bx, by, bw, click, cx, cy)
  ...
  local uiCfg = cfg.behavior.ui or {}
  local function hit(xx, wy, ww, hh)
    return inside(cx, cy, bx + xx, wy, ww, hh)
  end

  if hit(0, yy + z(18), z(44), z(26)) then
    uiCfg.scale = clamp((uiCfg.scale or 1.25) - 0.05, 0.75, 2.0)
    root._uiScale = uiCfg.scale
    cfg.behavior.ui = uiCfg
    Persistence.mark_dirty(root)
  end
  if hit(z(52), yy + z(18), z(44), z(26)) then
    uiCfg.scale = clamp((uiCfg.scale or 1.25) + 0.05, 0.75, 2.0)
    root._uiScale = uiCfg.scale
    cfg.behavior.ui = uiCfg
    Persistence.mark_dirty(root)
  ...
  if hit(z(104), yy + z(18), z(44), z(26)) then
    j.scanMaxDelay = math.max(1.0, (j.scanMaxDelay or 7) - 0.5)
  end
  if hit(z(156), yy + z(18), z(44), z(26)) then
  ...
  yy = yy + z(52)
  if hit(0, yy + z(18), z(160), z(28)) then
...
function M.render_items_tab(root, bx, by, bw, bottomY, C, V, draw_button)
  ...
  local function ufs(px)
    return math.max(8, math.floor((px or 12) * (root._uiScale or 1) + 0.5))
  end
  local rowH = z(ITEM_ROW_H)
  local listTop = by + z(LIST_TOP_OFFSET)
  local listH = math.max(z(80), bottomY - listTop - z(8))
  ...
  pcall(function()
    core.graphics.text_2d(
      "Changes save to loader scripts_data/ScienceAHBot/user_settings.lua (~1s after edits).",
      V(bx, by),
      ufs(12),
      C(160, 170, 195, 255)
    )
    core.graphics.text_2d(
      "Click bar, type digits 0-9, Backspace. Add sets ratio for new or existing ID.",
      V(bx, by + z(16)),
      ufs(11),
      C(130, 140, 165, 255)
    )
    ...
    core.graphics.rect_2d_filled(V(bx, by + z(34)), bw, z(24), C(28, 30, 40, 240), 3)
    core.graphics.text_2d(entry, V(bx + z(6), by + z(38)), ufs(14), ecol)

    draw_button(bx, by + z(62), 132, 28, "Merge starter")
    draw_button(bx + z(140), by + z(62), 88, 28, "Clear all")
    core.graphics.text_2d(string.format("New ratio: %.2f", ratio), V(bx + z(240), by + z(68)), ufs(12), C(200, 210, 230, 255))
    draw_button(bx + bw - z(160), by + z(62), 36, 28, "-")
    draw_button(bx + bw - z(118), by + z(62), 36, 28, "+")
    draw_button(bx + bw - z(200), by + z(94), 200, 30, "Add / update item")
  ...
  for i = 1, #ids do
    local id = ids[i]
    local row = cfg.Items[id]
    local ry = listTop + (i - 1) * rowH - scroll + z(6)
    if ry + rowH >= listTop and ry <= listTop + listH then
      pcall(function()
        local nm = (row and row.name) or "?"
        local r = (row and row.ratio) or 0.75
        core.graphics.text_2d(string.format("%d  %s", id, nm), V(bx + z(6), ry), ufs(13), C(210, 215, 230, 255))
        core.graphics.text_2d(string.format("%.2f", r), V(bx + z(188), ry), ufs(13), C(180, 220, 255, 255))
        draw_button(bx + z(188), ry - z(4), 36, 26, "-")
        draw_button(bx + z(230), ry - z(4), 36, 26, "+")
        draw_button(bx + bw - z(40), ry - z(4), 32, 26, "X")
      end)
    end
  end
  pcall(function()
  ...
  if #ids == 0 then
    pcall(function()
      core.graphics.text_2d("No items — merge starter or add an ID.", V(bx + z(8), listTop + z(12)), ufs(12), C(150, 155, 175, 255))
    end)
  end
end
...
function M.render_setup_tab(root, bx, by, bw, C, V, draw_toggle, draw_button)
  ...
  local uiCfg = b.ui or {}
  local scShow = uiCfg.scale or root._uiScale or 1

  local yy = by
  pcall(function()
    core.graphics.text_2d(
      string.format("UI scale: %.0f%% (− smaller, + larger)", scShow * 100),
      V(bx, yy - z(2)),
      ufs(12),
      C(180, 220, 255, 255)
    )
  end)
  draw_button(bx, yy + z(18), 44, 26, "-")
  draw_button(bx + z(52), yy + z(18), 44, 26, "+")
  yy = yy + z(52)

  pcall(function()
    core.graphics.text_2d("Setup (persists with Items to scripts_data/ScienceAHBot/)", V(bx, yy - z(2)), ufs(12), C(180, 200, 255, 255))
  end)
  yy = yy + z(16)
  ...
  pcall(function()
    core.graphics.text_2d("Min gold reserve: " .. fmt_gold(b.reserves.minGoldCopper), V(bx, yy), ufs(13), C(210, 215, 230, 255))
  end)
  draw_button(bx, yy + z(18), 44, 26, "-")
  draw_button(bx + z(52), yy + z(18), 44, 26, "+")
  yy = yy + z(52)
  ...
  pcall(function()
    core.graphics.text_2d(string.format("Default buy ratio (Items w/o ratio): %.2f", th.defaultBuyRatio or 0.75), V(bx, yy), ufs(13), C(210, 215, 230, 255))
  end)
  draw_button(bx, yy + z(18), 44, 26, "-")
  draw_button(bx + z(52), yy + z(18), 44, 26, "+")
  yy = yy + z(52)
  ...
  pcall(function()
    core.graphics.text_2d(string.format("Snipe max vs DBMarket: %.2f", b.snipe.maxBuyRatio or 0.52), V(bx, yy), ufs(13), C(210, 215, 230, 255))
  end)
  draw_button(bx, yy + z(18), 44, 26, "-")
  draw_button(bx + z(52), yy + z(18), 44, 26, "+")
  yy = yy + z(52)
  ...
  pcall(function()
    core.graphics.text_2d("Sell post stack size: " .. tostring(b.sell.postStackSize or 20), V(bx, yy), ufs(13), C(210, 215, 230, 255))
  end)
  draw_button(bx, yy + z(18), 44, 26, "-")
  draw_button(bx + z(52), yy + z(18), 44, 26, "+")
  yy = yy + z(52)
  ...
  pcall(function()
    core.graphics.text_2d(string.format("Buy scan mean: %.1f s", j.scanMeanSeconds or 5), V(bx, yy), ufs(13), C(210, 215, 230, 255))
  end)
  draw_button(bx, yy + z(18), 44, 26, "-")
  draw_button(bx + z(52), yy + z(18), 44, 26, "+")
  yy = yy + z(52)
  ...
  draw_button(bx, yy + z(18), 44, 26, "-")
  draw_button(bx + z(52), yy + z(18), 44, 26, "+")
  draw_button(bx + z(104), yy + z(18), 44, 26, "-")
  draw_button(bx + z(156), yy + z(18), 44, 26, "+")
  yy = yy + z(52)
  ...
  pcall(function()
    core.graphics.text_2d("Undercut (copper): " .. tostring(b.undercut.undercutCopper or 1), V(bx, yy), ufs(13), C(210, 215, 230, 255))
  end)
  draw_button(bx, yy + z(18), 44, 26, "-")
  draw_button(bx + z(52), yy + z(18), 44, 26, "+")
  yy = yy + z(52)
  ...
  yy = yy + z(34)
  pcall(function()
    core.graphics.text_2d(string.format("Learn blend: %.2f", L.blend or 0.35), V(bx, yy), ufs(12), C(200, 215, 235, 255))
  end)
  draw_button(bx, yy + z(18), 44, 26, "-")
  draw_button(bx + z(52), yy + z(18), 44, 26, "+")
  yy = yy + z(52)
  ...
  yy = yy + z(52)
  pcall(function()
    core.graphics.text_2d(string.format("EWMA alpha (react speed): %.2f", L.ewmaAlpha or 0.15), V(bx, yy), ufs(12), C(200, 215, 235, 255))
  end)
  draw_button(bx, yy + z(18), 44, 26, "-")
  draw_button(bx + z(52), yy + z(18), 44, 26, "+")
  yy = yy + z(52)
  draw_button(bx, yy + z(18), 160, 28, "Reset learned patterns")
  yy = yy + z(52)
