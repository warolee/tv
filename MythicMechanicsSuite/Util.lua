--[[ MythicMechanicsSuite — Util: pcall fault boundary + throttled error logging.
...
function M.safe_call(label, fn, ctx)
  ...
  local head = "[MythicMechanicsSuite] " .. tostring(label) .. " failed: " .. tostring(err)
  pcall(function()
...
function M.try(label, fn, ctx)
  ...
  if t - last >= throttle then
    ...
    pcall(function()
      if core and core.log_warning then
        core.log_warning("[MythicMechanicsSuite] " .. tostring(label) .. " failed: " .. tostring(err))
      end
    end)
  end
  return false, err
